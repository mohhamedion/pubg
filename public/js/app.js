...
var SELECT_CURRENCY_ROUTE = '/currency';
var SELECT_LOCALE_ROUTE = '/locale';

var toggle_select_currency = $('#toggle_select_currency');
var toggle_select_lang = $('#toggle_select_lang');
var toggle_support = $('#toggle_support');

var div_select_currency = $('div#select_currency_mod');
var div_select_lang = $('div#select_lang_mod');
var div_support = $('div#support_mod');

toggle_select_currency.click(function () {
    div_select_lang.hide();
    div_support.hide();
    div_select_currency.toggle();
});

toggle_select_lang.click(function () {
    div_select_currency.hide();
    div_support.hide();
    div_select_lang.toggle();
});

toggle_support.click(function () {
    div_select_currency.hide();
    div_select_lang.hide();
    div_support.toggle();
});

var select_currency = $('.selectCurrency');
var select_locale = $('.selectLanguage');
...
$(document).ready(function () {
    ...
    var collapse_button = $('#collapse-navbar');
    var nav = $('nav.nav');
    var headerLogoside = $('.header-logoside');
    var content = $('.content');
    ...
    collapse_button.click(function () {
        ...
        var collapsed = navbarCollapsed(nav);
        if (collapsed) {
...
function changeTheme() {
    ...
    var route = '/themes/change?theme=' + theme;
    $.ajax({
        url: route,
        type: 'GET',
...
!function (a) {
  ...
  }, a.fn.datepicker.version = "1.7.0-RC3", a.fn.datepicker.deprecated = function (a) {
...
(function (a) {
  if (true) {
    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(1)], __WEBPACK_AMD_DEFINE_FACTORY__ = (a),
				__WEBPACK_AMD_DEFINE_RESULT__ = (typeof __WEBPACK_AMD_DEFINE_FACTORY__ === 'function' ?
				(__WEBPACK_AMD_DEFINE_FACTORY__.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__)) : __WEBPACK_AMD_DEFINE_FACTORY__),
				__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
  } else {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_noty__ = __webpack_require__(0);
...
__WEBPACK_IMPORTED_MODULE_0_noty___default.a.overrideDefaults({
    layout: 'topRight',
    theme: 'metroui',
...
$(document).ready(function () {
    var package_name_group = $('.form-group__package_name');
    var package_name_input = $('#package_name');
    var find_app_button = $('#find_app');
    var application_preview = $('.application_preview');
    ...
    find_app_button.click(function () {
        ...
        if (package_name.length > 0) {
            switch (device_type) {
                case 'android':
                    $.get("/apps/android-info?query=" + package_name, function (_ref) {
                        var app = _ref.app;
...
var country_group_select = $('#country_group');
...
function updateCountryInput() {
    var group = country_group_select.val();
    var countries = country_group_select.data('groups')[group];
    country_select.empty();
    city_select.empty();
    $.each(countries, function (countryId, countryName) {
        country_select.append($("<option></option>").attr("value", countryId).text(countryName));
    });
...
function updateCityInput() {
    ...
    if (parseInt(countryId) !== 0) {
        ...
        $.get('/country/' + countryId + '/' + group + '/cities', function (data) {
            $.each(data, function (value, key) {
                city_select.append($("<option></option>").attr("value", value).text(key));
            });
        });
    }
}
 ...
 */
country_group_select.change(function () {
    updateCountryInput();
});
...
var time_delay_select = $('#time_delay');

city_select.change(function (_ref2) {
...
body.on('click', '.btn-cancel-task', function () {
    var id = $(this).data('id');
    var amount = $(this).data('amount') ? parseFloat($(this).data('amount')) : 0;
    var container = $(this).parent().parent().parent().parent(); // Get whole .form-item div
    var lang = $('#toggle_select_lang').data('lang');
    var message = 'Funds will back on Your balance ';
    if (lang === 'ru') {
        message = 'Средства будут возвращены на Ваш баланс ';
    }
    var old_balance = parseFloat($('#balance-value').html());
    var new_balance = amount + old_balance;
    bootbox.confirm(message + amount, function (result) {
        if (result) {
            $.ajax({
                url: '/apps/cancel/' + id,
                method: 'POST',
...
body.on('click', '.moderate-app', function () {
    ...
    $.ajax({
        ...
        success: function success() {
            ...
            var moderatingAppsLabel = $('#moderating_apps');
            var old_val = parseInt(moderatingAppsLabel.html());
            if (old_val > 1) {
...
$('.refresh-app').click(function (_ref3) {
    var target = _ref3.target;

    var self = $(target);
    self.addClass('fa-spin');

    var package_name = application.package_name; // Application variable involved at view file via PHP
    var package_country = $("#country option:selected").text();
    if (package_name.length > 0 && package_country.length > 0) {
        switch (application.device_type) {
            case 'android':
                $.get("/apps/android-info?query=" + package_name + "&lang=" + package_country + "&country=" + package_country, function (_ref4) {
                    var app = _ref4.app;
...
$('#country').on('change', function () {
    ...
    if (package_name.length > 0 && package_country.length > 0) {
        switch (application.device_type) {
            case 'android':
                $.get("/apps/android-info?query=" + package_name + "&lang=" + package_country + "&country=" + package_country, function (_ref5) {
                    var app = _ref5.app;
...
function loadParams() {
    var group = country_group_select.val();
    var customParams = $('.custom-params');
    ...
    $.get('/apps/edit/' + appId + '/load?group=' + group, function (html) {
...
function loadParamsAfter() {
    ...
    $('.custom-params-wrapper').html('');
    // Depending on selected load custom parameters HTML
    $.get('/apps/edit/' + appId + '/load/after?group=' + group + '&time_delay=' + time_delay, function (html) {
        customParams.removeClass('is-loading');
        $('.custom-params-wrapper').html(html);
    });
}
...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;/*! Select2 4.0.5 | https://github.com/select2/select2/blob/master/LICENSE.md */!function(a){ true?!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(1)], __WEBPACK_AMD_DEFINE_FACTORY__ = (a),
				__WEBPACK_AMD_DEFINE_RESULT__ = (typeof __WEBPACK_AMD_DEFINE_FACTORY__ === 'function' ?
				(__WEBPACK_AMD_DEFINE_FACTORY__.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__)) : __WEBPACK_AMD_DEFINE_FACTORY__),
...
$(document).ready(function () {
    var date_from = $('input[name=date_from].app-chart ');
    var date_to = $('input[name=date_to].app-chart ');
...
function updateChart(params) {
    var route = '/apps/data/chart';

    $.ajax({
        url: route,
        data: params,
        success: function success(chart) {
...
$('document').ready(function () {
    var date_from = $('input[name=date_from].roulette-date  ');
    var date_to = $('input[name=date_to].roulette-date  ');
...
$(document).ready(function () {
    ...
    if (wrapper.length) {
        ...
        var promocodes_select = $('select[name=promocodes]');

        var date_from_query_value = getQueryStringValue('date_from');
        var date_to_query_value = getQueryStringValue('date_to');
        var promocodes_value = getQueryStringValue('promocodes');
        ...
        promocodes_select.change(function () {
            ...
            var promocodes = promocodes_select.val();
        ...
        date_from_input.on('changeDate', function () {
            var date_from = date_from_input.val();
            var date_to = date_to_input.val();
            var promocodes = promocodes_select.val();
            var pathname = window.location.href.split('?')[0];
        ...
        date_to_input.on('changeDate', function () {
            var date_from = date_from_input.val();
            var date_to = date_to_input.val();
            var promocodes = promocodes_select.val();
            var pathname = window.location.href.split('?')[0];
...
$('select#role').select2({ search: false, minimumResultsForSearch: -1 });
...
$('.add-feature').click(function (_ref) {
...
$('.feature-delete').click(function (_ref2) {
    var target = _ref2.target;
...
$('.delete-offer').click(function (event) {
    ...
    var lang = $('#toggle_select_lang').data('lang');
    var message = 'Delete this special offer?';
    if (lang === 'ru') {
...
$('form[name=special-offer]').submit(function (event) {
    event.preventDefault();

    var form = $(event.target);
    var offer_id = form.data('id');
    var search_query = form.find('input[name=query]').val();
    var package_name = form.find('input[name=package_name]').val();
...
var special_offer_feature = '<div class="row">' + '<div class="col-xs-11">' + '<input name="features[]" class="form-control mb20"/>' + '</div>' + '<div class="col-xs-1 feature-delete-wrapper ">' + '<button title="Delete" type="button" class="feature-delete icon-button">' + '<i class="icons8-delete"></i>' + '</button>' + '</div>' + '</div>';
...
$('.payment-system-switch').on('change', function (event) {
    ...
    var saving_popup = new __WEBPACK_IMPORTED_MODULE_0_noty___default.a({
        type: 'info',
        text: '<i class="fa fa-circle-o-notch fa-spin fa-fw"></i> Saving'
    }).show();

    var self = $(this);
    var parent = self.closest('.payment-system');
    var id = $(this).data('id');
...
$('.system-top').on('change', function (_ref) {
    var target = _ref.target;

    var id = $(target).val();
    var route = "/payment-systems/" + id + "/top";
    $.ajax({
        url: route,
        type: 'GET',
        complete: function complete(response) {}
...
$('.system-active').on('change', function (_ref2) {
    var target = _ref2.target;

    var id = $(target).val();
    var route = "/payment-systems/" + id + "/active";
    $.ajax({
        url: route,
        type: 'GET',
        complete: function complete(response) {}
...
$('.task-review-app-title').click(function () {
    var self = $(this);
    var id = $(this).data('id');
    var route = id ? '/' + id + '/reviews' : '/reviews';
    var items_wrapper = self.parent().siblings($('.task-review-items'));
...
$('body').on('click', '.moderate-review', function () {
    ...
    var id = self.data('id');
    var reason = self.data('reason');
    var badge = self.parent().parent().parent().siblings($('.title-line')).find($('.task-review-app-title')).find($('.badge'));
    var old_badge_val = parseInt(badge.html());

    $('.moderate-review').prop('disabled', true);
    self.html(self.html() + '<i class="fa fa-spinner fa-pulse fa-fw"></i>');

    var moderating_reviews_badge = $('#moderating_reviews');
    var old_val = parseInt(moderating_reviews_badge.html());
    ...
    $.ajax({
        ...
        success: function success() {
            $(_this).closest('.task-review').hide(); // Remove whole parent element
            ...
            var new_badge_val = old_badge_val - 1;
            if (new_badge_val < 1) {
                badge.remove();
            } else {
...
$('#sendPushNotification').click(function (_ref) {
    var target = _ref.target;

    var self = $(target);
    var url = self.data('url');
    var message = $('#pushMessage').val();
    var button_text = self.html();
...
$('#sendAppPushNotification').click(function (_ref2) {
    var target = _ref2.target;
    ...
    $.ajax({
        ...
        statusCode: {
            201: function _() {
                ...
                var prevCount = parseInt(counter.html());
                if (prevCount === 1) {
                    counter.remove();
                } else {
        ...
        error: function error(_error2) {
            new __WEBPACK_IMPORTED_MODULE_0_noty___default.a({
                type: 'warning',
                timeout: 4000,
                text: _error2.responseJSON.error
            }).show();
        }
    });
});
    ...
    $.ajax({
        ...
        error: function error(_error3) {
            new __WEBPACK_IMPORTED_MODULE_0_noty___default.a({
                type: 'warning',
                timeout: 4000,
                text: _error3.responseJSON.error
            }).show();
        }
    });
});
...
$(document).ready(function () {
    $('input[name=date_from][data-chart]').each(function (index, input) {
        var date_to = $(input).siblings('input[name=date_to]');
        date_to.datepicker('setStartDate', input.value);
    });

    $('input[name=date_to][data-chart]').each(function (index, input) {
        var date_from = $(input).siblings('input[name=date_from]');
        date_from.datepicker('setEndDate', input.value);
    });

    /** ASYNC LOAD OF CHARTS AFTER PAGE DISPLAYING */
    var usersTasksChartDiv = $('#users_task_chart');
    if (usersTasksChartDiv.length) {
        var _route = usersTasksChartDiv.data('route');
        updateChart(usersTasksChartDiv, _route, '', '');
    }

    var registerChartDiv = $('#register_chart');
    if (registerChartDiv.length) {
        var _route2 = registerChartDiv.data('route');
        updateChart(registerChartDiv, _route2, '', '');
    }

    var awardsChartDiv = $('#awards_chart');
    if (awardsChartDiv.length) {
        var _route3 = awardsChartDiv.data('route');
        updateChart(awardsChartDiv, _route3, '', '');
    }

    var earnedChartDiv = $('#earned_chart');
    if (earnedChartDiv.length) {
        var _route4 = earnedChartDiv.data('route');
        updateChart(earnedChartDiv, _route4, '', '');
    }

    var transactionsChartDiv = $('#transactions_chart');
    if (transactionsChartDiv.length) {
        var _route5 = transactionsChartDiv.data('route');
        updateChart(transactionsChartDiv, _route5, '', '');
    }

    var transactionsAmountChartDiv = $('#transactions_amount_chart');
    if (transactionsAmountChartDiv.length) {
        var _route6 = transactionsAmountChartDiv.data('route');
        updateChart(transactionsAmountChartDiv, _route6, '', '');
    }

    var locationsChartDiv = $('#locations_chart');
    if (locationsChartDiv.length) {
        var _route7 = locationsChartDiv.data('route');
        updateChart(locationsChartDiv, _route7, '', '');
    }

    var locationsPieChartDiv = $('#locations_pie_chart');
    if (locationsPieChartDiv.length) {
        var _route8 = locationsPieChartDiv.data('route');
        updateChart(locationsPieChartDiv, _route8, '', '');
    }
});
    ...
    $.ajax({
        ...
        error: function error(_error4) {
            ...
            new __WEBPACK_IMPORTED_MODULE_0_noty___default.a({
                type: 'warning',
                timeout: 4000,
                text: _error4.responseJSON.message
            }).show();
        }
    });
}
...
function loader() {
    return '<div class="chart-load-wrapper">' + '<div class="loader">' + '<svg class="circular" viewBox="25 25 50 50">' + '<circle class="path" cx="50" cy="50" r="20" fill="none" stroke-width="2" stroke-miterlimit="10"/>' + '</svg>' + '</div>' + '</div>';
}

function showError() {
    return '<div class="chart-error-wrapper">' + '<h2>Error!</h2>' + '<h3>Something went wrong</h3>' + '</div>';
}

/*_________________________*/

body.on('click', '.delete-user', function () {
    var route = '/users/edit/' + $(this).data('id');
    $.ajax({
        url: route,
        type: 'DELETE',
        success: function success() {
        ...
        error: function error(_error5) {
            new __WEBPACK_IMPORTED_MODULE_0_noty___default.a({
                type: 'warning',
                timeout: 4000,
                text: _error5.responseJSON.message
            }).show();
        }
    });
});
...
$('.partner-top').on('change', function (_ref) {
    var target = _ref.target;

    var id = $(target).val();
    var route = "/partners/" + id + "/top";
    $.ajax({
        url: route,
        type: 'GET',
        complete: function complete(response) {}
...
$('.partner-available').on('change', function (_ref2) {
    var target = _ref2.target;

    var id = $(target).val();
    var route = "/partners/" + id + "/available";
    $.ajax({
        url: route,
        type: 'GET',
        complete: function complete(response) {}
...
$('.video-top').on('change', function (_ref) {
    var target = _ref.target;

    var id = $(target).val();
    var route = "/videos/" + id + "/top";
    $.ajax({
        url: route,
        type: 'GET',
        complete: function complete(response) {}
...
$('.video-available').on('change', function (_ref2) {
    var target = _ref2.target;

    var id = $(target).val();
    var route = "/videos/" + id + "/available";
    $.ajax({
        url: route,
        type: 'GET',
        complete: function complete(response) {}
